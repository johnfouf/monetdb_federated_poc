...
async def initialize():
    ...
    if mservers[0] != servers.servers and mservers!= [None]:
        await clearall()
        dbpool.clear()
        dbpool['local'] = []
    ...
    db_objects['global']['dbname'] = servers.servers[0]  ## global database name - required by remote tables to connect to the remote database
    ...
    for i,db in enumerate(servers.servers[1:]):
        ...
        try:      
            ...
            db_objects['local'].append(local_node) 
        except:
            continue # for each local node an asynchronous connection object, the database name, and a blocking connection object
    return db_objects
