...
def broadcast(db_objects, globalresulttable, globalschema):
    for i,local_node in enumerate(db_objects['local']):
        local_node['con'].cmd("sDROP TABLE IF EXISTS %s;" %globalresulttable)
        local_node['con'].cmd("sCREATE REMOTE TABLE %s (%s) on 'mapi:%s';" %(globalresulttable, globalschema, db_objects['global']['dbname']))   
