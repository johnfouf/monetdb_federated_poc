...
class Scheduler:
    ...
    async def schedule(self):
        try:
            result = None
            await self.task_executor.createlocalviews(self.step)
    ...
    async def set_schema(self, schema):
        ...
        await self.task_executor.init_tables(self.step, schema['local'], schema['global'])
    ...
    async def run_local(self, step_local):
        ...
        await self.task_executor.task_local(self.local_schema, self.step)

    async def run_global(self, step, step_global):
        ...
        result = await self.task_executor.task_global(step, self.global_schema, step_global['sqlscript'])
        return result
