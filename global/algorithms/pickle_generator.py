...
class GeneratorSnapshot(object):
    ...
    def __init__(self, f_gen):
        ...
        for key, value in list(f_gen.gi_frame.f_locals.items()):
...
def pickle_generator_filename(f_gen, filename):
    ...
    next(f_gen)  # jump one ahead for not to repeat when unpickling
...
def reducefunction(func):
    return (restorefunction, (func.__name__, marshal.dumps(func.__code__)))
